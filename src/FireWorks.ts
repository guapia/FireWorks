...
namespace test {
    ...
    export class FireWorksBombView extends BaseView {
        public xs: number[];
        public ys: number[];
        public renderXs: number[];
        public renderYs: number[];
        public alpha: number = 1;
        constructor(cx: number, cy: number, angle: number, radius: number) {
            super();
        ...
        onDraw(canvas: CanvasRenderingContext2D): void {
            super.onDraw(canvas);
            let index: number = this.id;//Math.floor(Math.random() * 100);
    ...
    export class FireWorksBombView2 extends BaseView {
        public xs: number[];
        public ys: number[];
        public renderXs: number[];
        public renderYs: number[];
    ...
    export class FireAnimation extends Animation {
        ...
        onStartAnimation(canvas: CanvasRenderingContext2D, view: BaseView): void {
            if (view instanceof FireWorksBombView || view instanceof FireWorksBombView2) {
        ...
        onEndAnimation(canvas: CanvasRenderingContext2D, view: BaseView): void {
            if (view instanceof FireWorksBombView || view instanceof FireWorksBombView2) {
                view.renderXs = [];
                view.renderYs = [];
            }
    ...
    export class FireWorksContainer extends ViewGroup {
        renderX: number;
        renderY: number;
        x: number;
        y: number;
    ...
    export class FireLayout extends RootView {
        ...
        init() {
            ...
            for (let m = 0; m < 50; ++m) {
                ...
                let fireWorksContainer: FireWorksContainer = new FireWorksContainer();
                ...
                for (let i = 0; i < size; ++i) {
                    ...
                    if (m % 2 == 0) {
                        let fire: FireWorksBombView2 = new FireWorksBombView2(cx, cy, angle, radius + Math.random() * 100);
            ...
            for (let view of this.children) {
                ...
                setTimeout(() => {
                    if (view instanceof FireWorksContainer) {
                        ...
                        animation1.onEndAnimation = (canvas: CanvasRenderingContext2D, view: BaseView) => {
                            oldanimationEnd(canvas, view);
